(function ($) {
    ...
    // Record start time for minimum preloader display duration
    var preloaderStartTime = Date.now();
    var minPreloaderDuration = 2000; // 2 seconds minimum
    ...
    function preloader() {
        if ($('.preloader').length) {
            ...
            var remainingTime = Math.max(0, minPreloaderDuration - elapsedTime);
            
            // Wait for minimum duration if needed, then trigger door opening
            setTimeout(function() {
                // Add hide class to trigger door opening animation
                $preloader.addClass('hide');
                // After doors open, fade out completely
                setTimeout(function() {
